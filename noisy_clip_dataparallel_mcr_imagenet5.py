...
class ImageNetCLIPDataset(LightningDataModule):
    ...
    def setup(self, stage=None):
        train_data = ImageNet5(
        	root=self.hparams.dataset_dir,
            split="train",
            transform=None
        )
        self.val_data = ImageNet5(
            root=self.hparams.dataset_dir,
            split="val",
...
class NoisyCLIP(LightningModule):
    def __init__(self, args):
        ...
        elif self.hparams.dataset == "ImageNet5":
            self.N_val = 250 # Default ImageNet validation set, only 100 classes.
        ...
        if self.hparams.dataset != "ImageNet100" and self.hparams.dataset != "Imagenet-100" and self.hparams.dataset != "ImageNet5":
    ...
    def criterion(self, input1, input2, label=None, reduction='mean'):
        """
        Args:
        ...
        elif self.hparams.loss_type == 'mcr':
            if label is None:
                raise ValueError('Error: Label is None.')
            teacher_embeds = F.normalize(input1, dim=1)
            student_embeds = F.normalize(input2, dim=1)
            criterion_MCR = MCR2_binary_classwise(gam1=1.0, gam2=1.0, eps=0.01)
            loss, expand_term, compress_term= criterion_MCR(input1, input2, label, num_classes=5)
            # print('loss binary mcr:', loss.item())
            # print('expand term:', expand_term)
            # print('compress term:', compress_term)
        else:
            raise ValueError('Loss function not understood.')

        return loss/bsz if reduction == 'mean' and self.hparams.loss_type != 'mcr' else loss
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        return {'embed_clean': embed_clean, 'embed_noisy': embed_noisy, 'labels': labels}
    ...
    def training_step_end(self, outputs):
        ...
        loss = self.criterion(embed_clean_full, embed_noisy_full, labels)
...
def run_noisy_clip():
    ...
    logger = TensorBoardLogger(
        ...
        name='NoisyCLIP_mcr_imagenet5_Logs'
    )
